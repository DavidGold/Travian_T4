...
<form method="POST" name="snd" action="build.php">
    ...
    <table cellpadding=\"1\" cellspacing=\"1\" class=\"under_progress\">
        ...
        foreach($trainlist as $train) {
			...
			if ($TrainCount == 1 ) {
				$NextFinished = $generator->getTimeFormat($train['commence']+$train['eachtime']);
				echo "<span id=timer1>".$generator->getTimeFormat($train['commence']+($train['eachtime']*$train['amt']))."</span>";
			} else {
